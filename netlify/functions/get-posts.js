...
exports.handler = async (event) => {
  ...
  try {
    let url = `${SUPA_URL}/rest/v1/posts?order=created_at.desc&limit=${limit}&offset=${offset}&select=id,body,is_anonymous,category,likes,created_at,user_id`;
    ...
    const enriched = await Promise.all((posts || []).map(async (post) => {
      ...
      // Traer primera reply visible
      const firstReplyRes = await fetch(
        `${SUPA_URL}/rest/v1/post_replies?post_id=eq.${post.id}&order=created_at.asc&limit=1&select=id,body,is_anonymous,user_id,created_at`,
        { headers: { 'Authorization': `Bearer ${SERVICE_KEY}`, 'apikey': SERVICE_KEY } }
      );
